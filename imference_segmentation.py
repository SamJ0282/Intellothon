...
def xyz(imagePath):
    ...
    net = ie.read_network('road-segmentation-adas-0001.xml', os.path.splitext('road-segmentation-adas-0001.xml')[0] + ".bin") #MODEL
    ...
    net.batch_size = 1 #len(args.input) #IMAGE
    ...
    for i in range(n):
        ...
        if image.shape[:-1] != (h, w):
            log.warning("Image {} is resized from {} to {}".format(imagePath, image.shape[:-1], (h, w))) #IMAGE
    ...
    for batch, data in enumerate(res):
        ...
        #out_img = "out_{}.bmp".format(batch) #MANUAL CODE - added comment
        cv2.imwrite('out_img.jpg', classes_map) #MANUAL CODE - added comment
